...
if __name__ == "__main__":
    ...
    model = SVDDModel(frontend="rawnet").cuda()
    _, output = model(x)
    print(output.shape) # expected: torch.Size([4, 1])
    
    print("Testing Spectrogram Encoder")
    model = SVDDModel(frontend="spectrogram").cuda()
    _, output = model(x)
    print(output.shape) # expected: torch.Size([4, 1])
    
    print("Testing Mel-Spectrogram Encoder")
    model = SVDDModel(frontend="mel-spectrogram").cuda()
    _, output = model(x)
    print(output.shape) # expected: torch.Size([4, 1])
    
    print("Testing LFCC Encoder")
    model = SVDDModel(frontend="lfcc").cuda()
    _, output = model(x)
    print(output.shape) # expected: torch.Size([4, 1])
